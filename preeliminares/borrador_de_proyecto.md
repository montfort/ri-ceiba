<!--
Este archivo contiene el borrador de lo que queremos para este proyecto.
---
Autor: José Villaseñor Montfort [pepemontfort@gmail.com]
...
## Propuesta de IA para redacción de objetivos y descripciones

// Inicio de la Propuesta de IA //

# Proyecto "Ceiba - Reportes de Incidencias"

## 1. Descripción General

Aplicación web para la gestión de **Reportes de Incidencias** de la Unidad Especializada en Género de la SSC-CDMX.  
Objetivo: centralizar, organizar y dar trazabilidad a los reportes generados por agentes de policía, con supervisión jerárquica y auditoría completa.
...
- **Agente:**  
  - Crear y editar un Reporte de Incidencia (Tipo A).  
  - Entregar el reporte → pasa a estado *no editable*.  
  - Consultar su historial de reportes entregados.  

- **Supervisor:**  
  - Recibir y visualizar reportes entregados.  
  - Buscar y filtrar reportes por campos.  
  - Editar reportes (ej. agregar acciones tomadas).  
  - Exportar reportes a PDF o JSON.  

- **Administrador Técnico:**  
  - Alta, baja y gestión de usuarios (agentes y supervisores).  
  - Configuración de campos dependientes (`zona`, `sector`, `cuadrante`) y listas de sugerencias (`sexo`, `delito`, `tipoDeAtencion`).  
  - Gestión de auditoría (consultar registros de accesos y acciones).  

- **Revisor (para reportes automáticos):**  
  - Consultar lista de reportes automáticos diarios.  
  - Visualizar, editar y aprobar reportes generados por IA.  
  - Gestionar modelos de reporte en formato Markdown.  

---

## 3. Funcionalidades Principales

1. **Gestión de Reportes de Incidencia (Tipo A):**  
   - Creación, edición y entrega por agentes.  
   - Edición posterior por supervisores.  
   - Historial de reportes por usuario.  

2. **Configuración de Campos:**  
   - Campos jerárquicos dependientes: `zona → sector → cuadrante`.  
   - Campos con listas de sugerencias: `sexo`, `delito`, `tipoDeAtencion`.  
   - Configuración centralizada y compartida entre todos los tipos de formularios.  
   - Toda configuración debe quedar registrada en auditoría.  

3. **Reportes Automáticos Diarios:**  
   - Generación programada (ej. 6:00am) con datos de las últimas 24h.  
   - Contenido: estadísticas (totales, delitos más comunes, zonas críticas) + resumen narrativo generado por IA.  
   - Almacenamiento en base de datos.  
   - Envío por correo electrónico (Word adjunto, usando API externa).  
   - Visualización y edición por usuario Revisor.  

4. **Auditoría:**  
   - Registro de todas las operaciones (usuario, acción, fecha, origen).  
   - Inclusión de procesos automatizados (ej. generación de reportes, envíos de correo).  

---

## 4. Requisitos No Funcionales

- **Escalabilidad:** Capacidad para manejar grandes volúmenes de reportes.  
- **Seguridad:** Protección de datos sensibles, control de accesos y trazabilidad.  
- **Usabilidad:** Interfaz intuitiva y fácil de usar.  
- **Flexibilidad:** Posibilidad de agregar nuevos tipos de reportes sin reestructurar la base de datos.  

---

## 5. Flujo de Trabajo Resumido

1. Agente inicia sesión → crea Reporte Tipo A → entrega → queda no editable.  
2. Supervisor revisa reportes entregados → busca/filtra → edita → exporta.  
3. Administrador Técnico gestiona usuarios, configuraciones y auditoría.  
4. Revisor consulta reportes automáticos diarios → edita → valida → se envían por correo.  

---

// Fin de la Propuesta de IA //
...
- El backend deberá programarse en ASP.NET Core versión 10, y deberá seguir la arquitectura de Monolito Modular.
- El frontend deberá programarse usando el framework Angular y los componentes Angular Material. Para este frontend utiliza la arquitectura que mejor convenga.
